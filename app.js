...
let inventory = {
  blue: [], // Set of active categories (e.g., 'naval', 'air')
  red: [],
  blueUnits: [], // Set of active unit IDs
  redUnits: []
};
let blueName = "Blue";
let redName = "Red";
...
function showAssessmentModal(onConfirm) {
  ...
  const populateList = (teamKey, container, teamUnits) => {
      ...
      Object.keys(teamData).forEach(cat => {
          (teamData[cat] || []).forEach(unit => {
              // Only show units that were originally selected or are currently in play
...
function renderInventorySelection() {
  const renderTeam = (teamKey, containerId) => {
    ...
    // Categories map for display labels
...
function setupEventListeners() {
  ...
  document.getElementById("setup-form").addEventListener("submit", (e) => {
    ...
    inventory.blue = blueData.cats;
    inventory.blueUnits = blueData.ids;
    inventory.red = redData.cats;
    inventory.redUnits = redData.ids;
